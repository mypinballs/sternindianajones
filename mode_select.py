...
from super_quick_multiball import *
...
class Mode_Select(game.Mode):
        ...
        def reset_scenes(self):
            ...
            self.film_set_flags = [0,0,0,0]
            self.game.set_player_stats('film_set_flags',self.film_set_flags)
            self.film_set_id = 0
            self.all_scenes_complete = False
            self.super_qm_qualified = False
        ...
        def mode_started(self):
            ...
            self.film_set_flags = self.game.get_player_stats('film_set_flags')
            ...
            #check if the film completed bonus round is qualified
            #additional check here in case ball drained in mode and its was the last mode for a film set
            self.check_film_completed()
        ...
        def start_scene(self):
            if self.mode_enabled and self.game.get_player_stats('multiball_started')==False and self.game.get_player_stats('quick_multiball_running')==False and self.game.get_player_stats('super_quick_multiball_started')==False and self.game.get_player_stats('lock_in_progress')==False and self.game.get_player_stats('dog_fight_running')==False:
        ...
        def check_film_completed(self):
            self.super_qm_qualified = False

            if self.select_list[0]==1 and self.select_list[1]==1 and self.select_list[2]==1 and self.select_list[3]==1 and self.film_set_flags[0]==0:
                self.super_qm_qualified = True
                self.film_set_id = 0
            elif self.select_list[4]==1 and self.select_list[5]==1 and self.select_list[6]==1 and self.select_list[7]==1 and self.film_set_flags[1]==0:
                self.super_qm_qualified = True
                self.film_set_id = 1
            elif self.select_list[8]==1 and self.select_list[9]==1 and self.select_list[10]==1 and self.select_list[11]==1 and self.film_set_flags[2]==0:
                self.super_qm_qualified = True
                self.film_set_id = 2
            elif self.select_list[12]==1 and self.select_list[13]==1 and self.select_list[14]==1 and self.select_list[15]==1 and self.film_set_flags[3]==0:
                self.super_qm_qualified = True
                self.film_set_id = 3

            self.game.set_player_stats('super_qm_qualified',self.super_qm_qualified)


        def start_super_qm(self):
            if self.super_qm_qualified and self.game.trough.num_balls_in_play > 0:
                self.film_set_flags[self.film_set_id] = 1 #update the film set flag store
                self.game.modes.add(self.super_quick_multiball)
                self.mode_enabled = False
                self.super_qm_qualified = False
                self.game.set_player_stats('super_qm_qualified',self.super_qm_qualified) #update global sqm qualified flag
