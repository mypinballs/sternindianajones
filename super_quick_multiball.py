# Super Quick Multiball
# Bonus mode for completing a full film set of modes
...
class SuperQuickMultiball(game.Mode):

	def __init__(self, game, priority):
            super(SuperQuickMultiball, self).__init__(game, priority)
            self.log = logging.getLogger('ij.super_quick_mb')
            ...
            self.jackpot_base_value = 20000000 #20 mil
            self.jackpot_boost_value = 500000
            self.add_ball_value = 1000000
            self.balls_needed = 4

            self.lamps = ['templeArrow','cairoSwordsman','xMarksTheSpot']
        ...
        def reset(self):
            ...
            self.add_a_ball_active = False #add a ball enabled flag
            ...
            self.game.set_player_stats('super_quick_multiball_running',self.multiball_running)
            self.game.set_player_stats('super_quick_multiball_started',self.multiball_started)
            #self.game.set_player_stats('super_quick_multiball_ready',self.multiball_ready_flag)
...
#            self.delay(name='reset_map', delay=1, handler=self.reset_map)
        ...
        #    info_layer1 = dmd.TextLayer(42, 8, self.game.fonts['8x6'], "center", opaque=False)
        #    info_layer2 = dmd.TextLayer(42, 16, self.game.fonts['07x5'], "center", opaque=False)
        #    info_layer3 = dmd.TextLayer(42, 24, self.game.fonts['07x5'], "center", opaque=False)
        #
        #    self.multiball_ready_flag = True
        #    self.game.set_player_stats('super_quick_multiball_ready',self.multiball_ready_flag)
        #
        #    info_layer1.set_text("HIT BALL",color=dmd.CYAN)
        #    info_layer2.set_text("FOR SUPER",color=dmd.CYAN)
        #    info_layer3.set_text("QUICK MULTIBALL",color=dmd.CYAN)
        #
        ...
        def multiball_start(self,timer):

            anim = dmd.Animation().load(game_path+"dmd/super_quick_multiball.dmd")
            self.layer = dmd.AnimatedLayer(frames=anim.frames,hold=False,frame_time=3)
            self.delay(name='start_delay', event_type=None, delay=timer, handler=self.multiball)
            ...
            #start ball save
            self.game.ball_save.start(num_balls_to_save=self.balls_needed,allow_multiple_saves=True,time=10)
        ...
        def multiball(self):
            ...
            title_frame = dmd.Animation().load(game_path+"dmd/super_quick_multiball.dmd")
            ...
            #setup display
            bgnd = dmd.Animation().load(game_path+"dmd/super_qm_bgnd_running.dmd")
        ...
        def multiball_tracking(self):
            ...
            if self.balls_in_play==1: #and self.multiball_running and not self.game.get_player_stats('lock_in_progress'):
                ...
                self.game.set_player_stats('super_quick_multiball_running',self.multiball_running)
                self.game.set_player_stats('super_quick_multiball_started',self.multiball_started)
                self.game.set_player_stats('super_qm_qualified',False)
                #self.game.set_player_stats('super_quick_multiball_ready',self.multiball_ready_flag)
