...
class Adventure(game.Mode):
	...
	def __init__(self, game, priority):
        ...
        def eject_ball(self):
            ...
            #safety catch
            self.delay(name='eject_retry',delay=2,handler=self.check_eject)


        def check_eject(self):
            if self.game.switches.grailEject.is_active():
                if self.eject_try<10:
                    self.eject_ball()
                    self.eject_try+=1
                else:
                    self.game.log('Ball eject failed for some reason - Check device!')

            else: #all good reset
                self.cancel_delayed('eject_retry')
                self.eject_try = 0
